...
class DNSValidator(BaseTool):
    ...
    def run(self, domain: Optional[str] = None, record_type: str = "A", **kwargs) -> str:
        """Validate DNS resolution for a domain.

        Args:
            domain: The domain name to validate
            record_type: The type of DNS record to retrieve (e.g., "A", "AAAA", "MX", "TXT"). Defaults to "A".
        ...
        try:
            ...
            result_lines = [f"✅ {rt} records for {domain}:"]
            for rdata in answers:
                result_lines.append(f"- {rdata}")
            return "\n".join(result_lines)
        except dns.resolver.NoAnswer:
            return f"❌ No {rt} records found for {domain}."
        except dns.exception.DNSException as e:
            return f"❌ DNS query failed for {domain} ({rt}): {str(e)}"
