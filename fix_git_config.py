...
def main():
    ...
    result = subprocess.run("git config core.sparseCheckout", shell=True, capture_output=True, text=True)
    if result.returncode == 0 and result.stdout.strip().lower() == "true":
        print("📋 Sparse checkout is enabled")
        
        # 5. Disable sparse checkout to include all files
        choice = input("Would you like to disable sparse checkout to include all files? (y/n): ").lower()
        if choice == 'y':
            run_git_command(
